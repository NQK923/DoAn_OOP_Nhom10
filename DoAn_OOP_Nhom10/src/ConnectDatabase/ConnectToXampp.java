...
public class ConnectToXampp {
    ...
    public static List<account>findAllAccount(){
        ...
        try{
            ...
            ResultSet rs = statement.executeQuery(sql);
            while(rs.next()){
                account e = new account(rs.getString("Gmail"), rs.getString("Name"), rs.getString("Pass"));
                listAccount.add(e);
            }
        } catch (SQLException e) {
            System.out.println(e);
    ...
    public static void insertAccount(account acc){
        ...
        try {
            conn = DriverManager.getConnection(url, user, password);
            String sql = "insert into accountuser(Gmail, Name, Pass) values(?, ?, ?)";
            statement = conn.prepareCall(sql);
            statement = conn.prepareStatement(sql);
            statement.setString(1, acc.getGmail());
            statement.setString(2, acc.getName());
            statement.setString(3, acc.getPass());
            statement.executeUpdate();
        } catch (SQLException e) {
            System.out.println(e);
        } finally {
    ...
    public static void deleteAccount(String Gmail, String Name){
        Connection conn = null;
        PreparedStatement statement = null;
        String url = "jdbc:mysql://localhost:3306/test";
        var user = "root";
        ...
        try {
            conn = DriverManager.getConnection(url, user, password);
            if(Gmail.equals("")){
                String sql = "delete from accountuser where Name = ?";
                statement = conn.prepareCall(sql);
                statement.setString(1, Name);
                statement.execute();
            }else{
                String sql = "delete from accountuser where Gmail = ?";
                statement = conn.prepareCall(sql);
    ...
    public static void updatePassOfAccount(account acc){
        ...
        try {
            conn = DriverManager.getConnection(url, user, password);
            String sql = "update accountuser set Pass=? where Gmail = ? and Name = ?";
            statement = conn.prepareStatement(sql);
            System.out.println("Conncect to success up!!");
            statement.setString(1, acc.getPass());
            statement.setString(2, acc.getGmail());
    ...
    public static List<listFlower>findAllEdit(){
        List<listFlower> listF = new ArrayList<>();
        Connection conn = null;
        Statement statement = null; 
        String url = "jdbc:mysql://localhost:3306/test";
    ...
    public static void insertEdit(listFlower flower){
        Connection conn = null;
        PreparedStatement statement = null;
        String url = "jdbc:mysql://localhost:3306/test";
        var user = "root";
        ...
        try {
            conn = DriverManager.getConnection(url, user, password);
            String sql = "insert into edit(Code, Name, Type, Color, Amount, SubFlowerNum, PetalsFlowerNum, day, month, year) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            statement = conn.prepareCall(sql);
            statement = conn.prepareStatement(sql);
            statement.setString(1, flower.getCode());
            statement.setString(2, flower.getName());
